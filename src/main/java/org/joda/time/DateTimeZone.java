...
public abstract class DateTimeZone implements Serializable {
    ...
    public static DateTimeZone forTimeZone(TimeZone zone) {
        ...
        return parseCustomZoneId(id, convId);
    }

    /**
     * Helper method to parse custom GMT/UTC ids provided by JDK TimeZone.
     * Extracted to reduce complexity of forTimeZone method.
     */
    private static DateTimeZone parseCustomZoneId(String originalId, String convertedId) {
        String idToParse = (convertedId == null) ? originalId : convertedId;

        if (idToParse.startsWith("GMT+") || idToParse.startsWith("GMT-")) {
            idToParse = idToParse.substring(3);
            if (idToParse.length() > 2) {
                char firstDigit = idToParse.charAt(1);
                if (firstDigit > '9' && Character.isDigit(firstDigit)) {
                    idToParse = convertToAsciiNumber(idToParse);
                }
            }
        ...
        throw new IllegalArgumentException("The datetime zone id '" + originalId + "' is not recognised");
    }
